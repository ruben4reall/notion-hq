...
// Uses a slot system so code blocks are never re-processed by inline rules.
// ──────────────────────────────────────────────────────────────────────────────
function renderMarkdown(raw: string): string {
  ...
  // ── Fenced code blocks (slot first, nothing touches them again) ───────────
  s = s.replace(/```(\w*)\n?([\s\S]*?)```/g, (_, lang, code) => {
    const badge = lang
  ...
  // ── Blockquotes (multi-line: consecutive "> " lines merged) ──────────────
  s = s.replace(/((?:^> .+\n?)+)/gm, match => {
  ...
  // ── Task checkboxes (before regular list parsing) ─────────────────────────
  s = s
    .replace(/^- \[ \] (.+)$/gm, (_, item) =>
  ...
  return (
    <div data-tour="notes-workspace" style={{ height: 'calc(100dvh - 56px)', display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {filtered.length === 0 ? (
            <div style={{ padding: 24, textAlign: 'center' }}>
              <p style={{ fontSize: 12, color: 'var(--t2)', marginBottom: 12 }}>{search ? 'Aucun résultat' : 'Aucune note'}</p>
              {!search && <button onClick={create} className="btn btn-primary" style={{ fontSize: 12, padding: '6px 14px' }}>Créer une note</button>}
            </div>
          ) : filtered.map(note => (
            <div
              key={note.id}
      ...
      <div className="lg:hidden" style={{ position: 'absolute', top: 56, left: 0, right: 0, zIndex: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg-1)', borderBottom: '1px solid var(--border-s)' }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--t0)', flex: 1 }}>{active ? (active.titre || 'Sans titre') : 'Notes'}</span>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg-0)', minWidth: 0 }}>
          ...
          <div style={{ padding: '10px 20px', borderBottom: '1px solid var(--border-s)', display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg-1)', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, position: 'relative' }}>
                  ...
                  {showSharePicker && (
                    <>
                      ...
                      <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, zIndex: 50, background: 'var(--bg-2)', border: '1px solid var(--border-m)', borderRadius: 10, padding: 8, minWidth: 200, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', animation: 'slideDown 0.18s var(--ease-spring) both' }}>
                        <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--t2)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 8, padding: '0 4px' }}>Membres du projet</p>
                        {users.filter(u => u.name !== myName).length === 0 ? (
                          <p style={{ fontSize: 12, color: 'var(--t2)', padding: 8, textAlign: 'center' }}>Invitez des membres pour partager des notes.</p>
              ...
              {/* Split / preview toggles — desktop only */}
              <div className="hidden lg:flex" style={{ gap: 4, alignItems: 'center' }}>
                <button
                  onClick={() => setSplitView(v => !v)}
                  title={splitView ? 'Éditeur seul' : 'Vue partagée'}
          ...
          <div className="hidden lg:flex" style={{ padding: '5px 20px', borderBottom: '1px solid var(--border-s)', alignItems: 'center', gap: 2, background: 'var(--bg-1)', flexShrink: 0, flexWrap: 'wrap' }}>
            ...
            {/* Ordered list */}
            <button
              onMouseDown={e => { e.preventDefault(); insertFormat('1. ', '') }}
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
            {/* Textarea — shown in editor/split mode on desktop, edit mode on mobile */}
            <textarea
            ...
            {/* Live preview — right pane in split view, full in preview mode */}
            {(splitView || preview) && (
              <div
                style={{
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                  {(note.utilisateur !== myName || note.sharedWith.length > 0) && (
                    <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 4, background: note.utilisateur !== myName ? 'var(--accent-bg)' : 'var(--bg-3)', color: note.utilisateur !== myName ? 'var(--accent)' : 'var(--t2)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Partagée</span>
                  )}
